...
import static com.bignerdranch.android.MyReceipts.database.ReceiptDbSchema.ReceiptTable.Cols.*;
...
public class ReceiptLab {
    ...
    public void deleteReceipt(UUID receiptId) {
        ...
        mDatabase.delete(ReceiptTable.NAME, Cols.UUID + " = ?", new String[]{uuidString});
    }
    ...
    private static ContentValues getContentValues(Receipt receipt) {
        ...
        values.put(DESCRIPTION, receipt.getDescription());
        values.put(SHOP, receipt.getShopName());
        values.put(LONGITUDE, receipt.getLatitude());
        values.put(LATITUDE, receipt.getLongitude());
        values.put(LOCATIONSET, receipt.getHasSetLocation());
